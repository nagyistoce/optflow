...
/**
 * Defines a motion extractor that uses coarse-to-fine approach 
 * with image pyramids. Each lower-resolution motion field is 
 * used as an initial guess for the next level. 
 * Each pyramidal motion extractor implementation should derive 
 * this class. This class implements functionality for both 
 * single and dual motion extractors. 
 * Each derived class must supply a single-resolution motion 
 * extractor for extracting the motion in each level.
 *
